...
export default function EditNoteModal({ noteId, onClose }: { noteId: string; onClose: () => void }) {
  const [form, setForm] = useState({
    title: "", description: "", course_code: "",
    department: "", university: "", faculty: "", visibility: "all",
  });
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="mx-auto w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-5 shadow-xl dark:bg-[#1e293b]">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-zinc-900 dark:text-white">Notu Düzenle</h2>
          ...
          {loading ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <input type="text" placeholder="Başlık" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required className="w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm outline-none focus:border-campus-500" />
