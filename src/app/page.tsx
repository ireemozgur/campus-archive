...
const projectPosts = teamPosts.slice(0, 3);
const communityPosts = teamPosts.slice(3);
  ...
  return (
    <div className="flex flex-col">
      ...
      {projectPosts.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 pb-24">
          <div className="mb-8 flex items-center justify-between">
            <h2 className="text-2xl font-bold dark:text-white">🤝 Projeye Ekip Arkadaşı Aranıyor</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {projectPosts.map((post: any) => (
              <Link key={post.id} href="/teammates" className="rounded-2xl border border-zinc-200 p-5 transition-all hover:border-campus-200 hover:shadow-sm dark:border-zinc-700 dark:hover:border-campus-700">
                <div className="mb-3 flex items-center justify-between">
                  <span className="rounded-lg bg-campus-50 px-2.5 py-1 text-xs font-medium text-campus-700 dark:bg-campus-800 dark:text-campus-200">{post.project_type}</span>
                  {post.team_size && <span className="text-xs text-zinc-400 dark:text-zinc-500">{post.team_size} kişi</span>}
                </div>
      ...
      {communityPosts.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 pb-24">
          <div className="mb-8 flex items-center justify-between">
            <h2 className="text-2xl font-bold dark:text-white">👥 Topluluğa Ekip Arkadaşı Aranıyor</h2>
            <Link href="/teammates" className="text-sm font-medium text-campus-600 hover:text-campus-700 dark:text-campus-400">
              Tümünü Gör →
            </Link>
          </div>
